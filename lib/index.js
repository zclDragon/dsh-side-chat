...
const name = "@zhuchenglong/dsh-side-chat";
...
*/
function sideChatTitleOf(session) {
	const boundary = session.header.seedLength ?? 0;
	const first = session.events.find((event) => event.type === "user/message" && event.data.source.kind === "user" && event.seq >= boundary);
	if (first === void 0) return void 0;
	const text = (first.data.content ?? []).filter((block) => typeof block === "object" && block !== null && block.type === "text").map((block) => block.text).join(" ").replace(/\s+/gu, " ").trim();
	...
	function listSideChats(parentSessionId) {
		...
		for (const [sideSessionId, record] of registry) {
			...
			rows.push({
				...
				title: side === void 0 ? "" : sideChatTitleOf(side) ?? ""
			});
		}
...
//#endregion
export { Config, PLUGIN_ID, ROUTE_PREFIX, SIDE_CHAT_TITLE_MAX, SIDE_COMMAND, apply, inject, name, sideChatSeed, sideChatTitleOf };
